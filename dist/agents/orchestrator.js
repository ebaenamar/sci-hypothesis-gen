...
export class AgentOrchestrator {
     ...
     */
    async runSequentialWorkflow(graphPath) {
     ...
     */
    async runFlexibleWorkflow(keywords, graphPaths) {
     ...
     */
    async runOntologist(graphPath) {
        const agent = this.agents.get('ontologist');
        if (!agent)
            throw new Error('Ontologist agent not found');
...
Provide a structured ontological analysis.`;
     ...
     */
    async runScientist1(graphPath, ontologyContext) {
        const agent = this.agents.get('scientist_1');
        if (!agent)
            throw new Error('Scientist 1 agent not found');
        const pathDescription = graphPath.nodes.map(n => n.label).join(' → ');
...
Think creatively about connections between these concepts.`;
     ...
     */
    async runScientist2(initialHypothesis) {
        const agent = this.agents.get('scientist_2');
        if (!agent)
            throw new Error('Scientist 2 agent not found');
...
Be constructive but thorough in identifying weaknesses.`;
     ...
     * Run planner agent
     */
    async runPlanner(keywords, graphPaths) {
...
REASONING: [explanation]`;
    ...
    parseCritiques(response) {
        ...
        for (const aspect of aspects) {
            const regex = new RegExp(`\\*\\*${aspect}\\*\\*[^]*?rating[^\\d]*(\\d+)[^]*?comments?:?\\s*([^]*?)(?=\\*\\*|suggestions?:?|$)`, 'i');
     ...
     */
    calculateAverageScore(critiques, aspect) {
